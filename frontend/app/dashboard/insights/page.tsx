...
export default function InsightsPage() {
  ...
  const exportToPDF = async () => {
    setIsExporting(true);
    showToast('Preparing your intelligent report...', 'info');
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          ...
          // 2. Instead of removing links, we'll try to let them stay but override problematic colors globally
          const el = clonedDoc.getElementById('insights-content');
          if (el) {
            // Setup professional report container
            ...
            header.innerHTML = `
              <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #3b82f6; padding-bottom: 20px; margin-bottom: 40px;">
                <div>
                  <h1 style="font-size: 32px; font-weight: 900; color: #1e293b; margin: 0; letter-spacing: -1px;">NEURAL INSIGHTS REPORT</h1>
                  <p style="font-size: 14px; color: #64748b; margin: 5px 0 0 0;">AI-Powered Decision Support System • Intelligence Module</p>
                </div>
            ...
            footer.innerHTML = `
              <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; font-size: 10px; color: #94a3b8; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
                <div>Confidential Intelligence Report • Internal Use Only</div>
                <div>Generated by Neural Core v2.0</div>
              </div>
            `;
            el.appendChild(footer);

            // Inject global overrides for Tailwind 4 variables
            ...
            // 3. Deep clean ALL elements in the entire document for inline styles and attributes
            ...
            for (let i = 0; i < allElements.length; i++) {
              ...
              // Hide export buttons and other UI elements
              if (item.innerText === 'Download PDF Report' || item.innerText === 'Share Insights') {
                item.style.display = 'none';
              }

              // Clean SVG specific attributes
              ...
              // Ensure all cards have proper white background and borders
      ...
      // Use dynamic import for jsPDF to ensure it's handled correctly
      ...
      pdf.save(`Neural-Insights-${dateStr}.pdf`);
  ...
  const exportToCSV = () => {
    try {
      ...
      if (!currentDataset) {
        showToast('No dataset available to export', 'error');
        return;
      }

      const headers = ['Metric', 'Primary Value', 'Importance', 'Analysis'];
      const rows = (currentDataset.metrics || []).map((m: any) => [
        `"${m.metric || 'Unnamed Metric'}"`, 
        `"${m.data?.values?.[0] || 'N/A'}"`, 
        m.importance || 'N/A',
        `"${(m.ai_reason || '').replace(/"/g, '""')}"`
      ]);
      ...
      showToast('CSV metrics exported successfully!', 'success');
    } catch (error) {
